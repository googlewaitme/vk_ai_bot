...
class DialogFlow:
    ...
    def get_answer(self, user_id, text_to_be_analyzed):
        ...
        if intent_name == 'KreditForm' and params['birthday']:
            if not self._is_adult(params.pb):
                return 'По законодательству РФ мы не в праве заключать ' \
                       'договора с гражданами не достигшими возраста ' \
                       '18 лет. Обратитесь позже.'
    ...
    def _is_adult(self, params):
        birthday_string = MessageToDict(params['birthday'])
        birthday_date = self._get_date(birthday_string)
        today = date.today()
        age = today - birthday_date
        eightteen = timedelta(days=364 * 18)
        return eightteen < age
